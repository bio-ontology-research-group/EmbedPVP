...
with open("../data/phenotype_annotation.tab","r") as f:
    for line in f.readlines():
        ...
        if (data[0]=="OMIM")&(data[5][:4]=="OMIM"):
            if data[5].strip() not in dis_phe:
...
mgi_do=pd.read_table("/ibex/scratch/althagat/intel/JunWork/DL2vec/data/MGI_DO.rpt.txt", sep='\t')#/ibex/scratch/althagat/intel/JunWork/DL2vec
human_gene_disease=[]
human_gene_disease2=[]
...
print("the number of human_gene_disease ",len(human_gene_disease))
...
print("the number of mouse_gene_disease ",len(mouse_gene_disease))
...
print("the length of mouse_to_human",len(mouse_to_human.keys()))
...
print("the length of gene and go association",len(gene_go_feature))
print("total total_gene_go_id id",len(total_gene_go_id))
#print(gene_go_feature)
...
print("the length of mouse gene and phenotypes", len(mp_pheno))
print("total mouse gene before",len(total_mouse_gene_before))

# hp
gene_hp_feature=dict()
...
with open('../data/genes_to_phenotype.txt') as f:
    ...
    for line in f:
        ...
        gene = l[0]
        hpo = l[2]
        if gene not in gene_hp_feature:
            gene_hp_feature[gene]=[hpo]
        else:
            gene_hp_feature[gene].append(hpo)

print("the length of human gene and hp phenotypes", len(gene_hp_feature))
...
with open("../data/cl_gene_dic.pkl","rb") as f:
    gene_cl_feature=pkl.load(f)

#gene_cl_feature : my file data/CL dic 
            ...
            if data in gene_cl_feature.keys():
                if data in gene_hp_feature.keys():
                    ...
                    for value in gene_uberon_feature[data]:
                        features.add(value)
                    for value in gene_cl_feature[data]:
                        features.add(value)
                    for value in gene_hp_feature[data]:
                        features.add(value)
                    ...
                    for value in gene_cl_feature[data]:
                        cl_intersection_features.add(value)
                    for value in gene_hp_feature[data]:
                        hp_intersection_features.add(value)
                    ...
                    gene_cl_intersection_feature[data]=cl_intersection_features
                    gene_hp_intersection_feature[data]=hp_intersection_features
...
for data in gene_mp_feature.keys():
    gene_total.add(data)
for data in gene_cl_feature.keys():
    gene_total.add(data)
for data in gene_hp_feature.keys():
    gene_total.add(data)
...
print("num of cl    ",str(len(gene_cl_feature)))
print("num of hp    ",str(len(gene_hp_feature)))
...
print("number of cl in intersection",str(len(gene_cl_intersection_feature)))
print("number of hp in intersection",str(len(gene_hp_intersection_feature)))
...
def generate_features(mouse_gene_disease_association,gene_feature,disease_feature,type):
    print(type)
    file1=open("../data/"+type+"_associations.txt","w")
    disease_gene=dict()
    entity_List=[]
    count=0
    ...
    for gene in gene_feature.keys():
        ...
        entity_List.append(gene)
        for feature in features:
            feature=feature.replace(":","_")
            file1.write(gene+" "+'<http://purl.obolibrary.org/obo/'+str(feature)+">"+"\n")
    ...
    with open("../data/associtions.txt","r") as f:
        for line in f.readlines():
            l = line.split("\t")
            file1.write(l[0]+" "+l[1])
            entity_List.append(l[0])
    ...
    print('disease_gene: ',len(disease_gene))    
    
    with open("../data/"+type+"_disease_gene.pkl","wb") as f:
        pkl.dump(disease_gene,f)
    with open("../data/"+type+"ـgene_set.pkl","wb") as f:
        pkl.dump(total_gene,f)
    with open("../data/"+type+"_entity_List.pkl","wb") as f:
        pkl.dump(entity_List,f)
        
    print("the num of gene disease association :",str(count))
    print(" the number of total gene",len(total_gene))
    print(" the number of entity_List",len(entity_List))
...
generate_features(mouse_gene_disease,gene_cl_feature,dis_phe,"cl")
generate_features(mouse_gene_disease,gene_hp_feature,dis_phe,"hp")
...
generate_features(mouse_gene_disease,gene_cl_intersection_feature, dis_phe,"cl_intersection")
generate_features(mouse_gene_disease,gene_hp_intersection_feature, dis_phe,"hp_intersection")
