...
@ck.option("--window", "-w", type=int, default=5)
@ck.option("--epochs", "-e", type=int, default=20)
@ck.option("--num_walks", "-n", type=int, default=10)
@ck.option("--walk_length", "-l", type=int, default=30)
...
def main(onto, method, vector_size, window, epochs, num_walks, walk_length, device):
    ...
    walker = DeepWalk(num_walks, walk_length, 0.1, workers=10, outfile=tmp + 'walk', seed=42)
    ...
    projector = TaxonomyWithRelationsProjector(taxonomy=False, relations=["http://is_associated_with"])
