...
def main(dataset, bench, onto, method, vector_size ,window, epochs,  num_walks , walk_length, device):
    ...
    graph_train_file = ROOT + f"graphs/{dataset}_{onto}_{bench}_{method}_train.pkl"
    graph_test_file = ROOT + f"graphs/{dataset}_{onto}_{bench}_{method}_test.pkl"
    ...
    eval_train_file = ROOT + f"eval_data/{dataset}_{onto}_{bench}_{method}_training_set.pkl"
    eval_test_file = ROOT + f"eval_data/{dataset}_{onto}_{bench}_{method}_test_set.pkl"
    eval_heads_file = ROOT + f"eval_data/{dataset}_{onto}_{bench}_{method}_head_entities.pkl"
    eval_tails_file = ROOT + f"eval_data/{dataset}_{onto}_{bench}_{method}_tail_entities.pkl"
    ...
    walker = DeepWalk(num_walks, # number of walks per node
                      ...
                      workers=10, outfile = walks_file,seed=42) # number of threads
    ...
    vectors = model.wv
    word2vec_file = ROOT + f"embeddings/{dataset}_{onto}_{bench}_{method}_embed.pkl"
